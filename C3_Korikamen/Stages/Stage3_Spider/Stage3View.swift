...
struct Stage3View: View { // 맥스 바보 
    ...
    // 우측 게이지 HUD (scene2일 때만)
    private var rightHUD: some View {
        Group {
    ...
    // 거미줄 이미지 2장 + 위→아래 와이프
    private var webImages: some View {
        ZStack {
    ...
    // 와이프 마스크 (위에서부터 거미줄이 걷히도록)
    private var webMask: some View {
        LinearGradient(
            stops: [
    ...
    // Scene1 화살표 힌트 (관을 왼 - 오로 움직이라는 시각적 피드백 추가)
    private var arrowHint: some View {
        Group {
